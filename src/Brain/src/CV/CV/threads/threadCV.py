...
def region_selection(image):
    ...
    top_left     = [cols * 0.005, rows * 0.55]
    bottom_right = [cols * 0.999, rows * 0.99]
    top_right    = [cols * 0.995, rows * 0.55]
...
def trajectory_line(left_line, right_line, frame_width):
    ...
    traj_line = {(int(frame_width/2), left_line[0][1]),(mean_x1, left_line[1][1])}
...
class threadCV(ThreadWithStop):
    """This thread handles CV.
    Args:
    ...
    def __init__(self, queueList, logging, debugging=False):
        ...
        debugging_g = debugging

        self.subscribe()
        super(threadCV, self).__init__()

    def run(self):
        LOG(" CV THREAD RUNNING")
        ...
        while True:
            ...
            if not ret:
                LOG("Error: Unable to capture video.")
                break
            ...
            if hough is not None and filter_weak_lines(hough):
                ...
                traj_line = trajectory_line(left_line, right_line, frame_width)
                # Draw the lane lines on the frame
                complete_image = draw_lane_lines(frame, [left_line, right_line])
                
                if traj_line is not None:
                    angle = get_line_angle(frame,traj_line)
                    self.trajectory_angle_pub.send(float(angle))
                    complete_image = draw_trajectory_line(complete_image, traj_line)
                    time.sleep(1)
                
                _, serialEncodedImg = cv2.imencode(".jpg", complete_image)
    ...
    def subscribe(self):
        self.trajectory_angle_pub = messageHandlerSender(self.queuesList, allMessages.Trajectory_angle_rads)

        self.recordingSender = messageHandlerSender(self.queuesList, allMessages.Recording)
        self.mainCameraSender = messageHandlerSender(self.queuesList, allMessages.mainCamera)
        self.serialCameraSender = messageHandlerSender(self.queuesList, allMessages.serialCamera)
